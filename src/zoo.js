...
function getOldestFromFirstSpecies(animalId) {
  // seu código aqui
  const firstAnimalResponsibleId = employees.find((employee) => employee.id === animalId).responsibleFor[0];
  let animalSelected = species.find(({ id }) => id === firstAnimalResponsibleId);
...
function increasePrices(percentage) {
  ...
  prices.Adult = Math.round((percentage * (Adult / 100) + Adult) * 100) / 100;
  prices.Child = Math.round((percentage * (Child / 100) + Child) * 100) / 100;
  prices.Senior = Math.round((percentage * (Senior / 100) + Senior) * 100) / 100;
}

function getEmployeeCoverage(idOrName) {
  let result = {}
  if (!idOrName) {
    employees.forEach(employer => { result[`${employer.firstName} ${employer.lastName}`] = employer.responsibleFor.map((animalId) => species.find(({ id }) => id === animalId).name)});
  } else {
    let employerSelected = employees.find(({ firstName, lastName, id }) => idOrName === firstName || idOrName === lastName || idOrName === id);
    result[`${employerSelected.firstName} ${employerSelected.lastName}`] = employerSelected.responsibleFor.map((animalId) => species.find(({ id }) => id === animalId).name);
  }
  return result;
}
